/* ------------------------------------------------------------------
   Three’s Card Game – Rule Engine  (per‑player notice + auto‑pickup)
   ------------------------------------------------------------------ */
...
export class Game {
  ...
  removePlayer(sock){ this.players = this.players.filter(p => p.id !== sock.id); }

  /* ---------- public actions ---------- */
  play(sock, idxs){
    const p = this.byId(sock.id); if(!p || this.turn !== p.id) return;
  ...
  deal(){ for(let i=0;i<3;i++) this.players.forEach(p=>{ p.down.push(this.draw()); p.up.push(this.draw()); p.hand.push(this.draw()); }); }
  ...
  givePile(p,msg){
    ...
    if(this.deck.length) this.playPile.push(this.draw());
    if(p.sock) p.sock.emit('notice', msg);     // only to that player
    this.advanceTurn();
  }
  autoLoop(){
    ...
    while(guard--){
      ...
      if(this.hasMove(p)) break;
      this.givePile(p,'You had no moves and picked up the pile');
    }
  }
  ...
  reset(){ this.players=[]; this.deck=[]; this.playPile=[]; this.discard=[]; this.turn=null; this.started=false; }
}
